# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 16:53:38 2024
...
def genetic_algorithm(players, pick_order, pop_size=50, generations=200):
    ...
    for generation in range(generations):
        ...
        agent_history.append(population)
        generations_history.append(generation + 1)
        # Plot update
        ax.clear()
        ax.plot(generations_history, agent_history, marker='o', linestyle='-')
        ax.set_xlabel('Generation')
        ax.set_ylabel('Agent Score')
        ax.set_title('Real-Time Scoring Progress')
        plt.draw()
        plt.pause(0.01)
